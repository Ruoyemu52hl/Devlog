...
Python 后端负责“编排”：接收玩家输入和空间状态，读取记忆与关系状态，构建 Prompt，调用 LLM，解析结构化输出，调用 MiniMax TTS，把音频转换成 Unity 可流式播放的 PCM，再通过 SSE 返回。
...
历史摘要则用于压缩更早的内容。对话轮数增加后，系统会在后台触发摘要任务，把若干轮对话总结成更短的长期信息，同时提取可能有长期价值的记忆，比如玩家偏好、关系事件、项目设定等。
...
这比单纯的数值变化更适合角色表现。因为同样是 50 点好感，如果刚刚连续发生了负向互动，角色应该明显更冷淡；如果刚刚连续发生正向互动，角色则可以表现得更愿意靠近。

关系系统还参与表现约束。比如低好感阶段，如果玩家突然提出亲密要求，后端会把这类输入识别为边界压力，不让角色输出过于甜蜜的回应。这能避免“数值还很低，但角色突然变得很亲密”的违和感。

## TTS 流式语音

语音部分使用 MiniMax TTS WebSocket。后端启动时会初始化一个 WebSocket 连接池，默认可以配置多个连接。每当 LLM 输出一句 `sentence_fast`，后端就创建一个 TTS 任务，把这句话送给 MiniMax。

MiniMax 返回的是音频片段。后端拿到 MP3 chunk 后，会用 `miniaudio` 增量解码成 `pcm_s16le`。之所以在后端转成 PCM，是因为 Unity 端可以更直接地把 PCM 样本写进流式播放缓冲，不需要在客户端再处理 MP3 解码问题。

整体流程是：

```text
sentence_fast -> TTS WebSocket -> MP3 chunk -> PCM chunk -> base64 -> SSE -> Unity
```
